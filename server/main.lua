...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        if (v.PlayerData.job.name == "statetrooper" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("statetrooper:client:UpdateBlips", -1, dutyPlayers)
end
...
QBCore.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    ...
    if Player then
        if Player.PlayerData.job.name == "statetrooper" and Player.PlayerData.job.onduty then
            TriggerClientEvent('statetrooper:client:SpawnSpikeStrip', src)
        end
    end
end)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "statetrooper" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "statetrooper" and Player.PlayerData.job.onduty then
        if type == "cone" then
            TriggerClientEvent("statetrooper:client:spawnCone", src)
        elseif type == "barrier" then
            TriggerClientEvent("statetrooper:client:spawnBarrier", src)
        elseif type == "roadsign" then
            TriggerClientEvent("statetrooper:client:spawnRoadSign", src)
        elseif type == "tent" then
            TriggerClientEvent("statetrooper:client:spawnTent", src)
        elseif type == "light" then
            TriggerClientEvent("statetrooper:client:spawnLight", src)
...
QBCore.Commands.Add("escort", Lang:t("commands.escort"), {}, false, function(source, args)
    local src = source
    TriggerClientEvent("statetrooper:client:EscortPlayer", src)
end)
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {{name = "id", help = Lang:t('info.player_id')}, {name = "time", help = Lang:t('info.jail_time')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "statetrooper" and Player.PlayerData.job.onduty then
        ...
        if time > 0 then
            TriggerClientEvent("statetrooper:client:JailCommand", src, playerId, time)
        else
    ...
    if Player.PlayerData.job.name == "statetrooper" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                OtherPlayer.Functions.AddMoney("bank", 500, "statetrooper-tow-paid")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "statetrooper" or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
        if OtherPlayer then
        ...
        if args[1] then
            ...
            if Target then
                if Target.PlayerData.metadata["tracker"] then
                    TriggerClientEvent("statetrooper:client:SendTrackerLocation", Target.PlayerData.source, src)
                else
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if ((Player.PlayerData.job.name == "statetrooper") and Player.PlayerData.job.onduty) and OtherPlayer then
...
RegisterNetEvent('statetrooper:server:SendTrackerLocation', function(coords, requestId)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    ...
    if Player.Functions.GetItemByName(item.name) then
        if item.info and item.info ~= "" then
            if Player.PlayerData.job.name ~= "statetrooper" then
...
-- Callbacks
QBCore.Functions.CreateCallback('statetrooper:server:isPlayerDead', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('statetrooper:GetPlayerStatus', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('statetrooper:IsSilencedWeapon', function(source, cb, weapon)
...
QBCore.Functions.CreateCallback('statetrooper:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "statetrooper" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('statetrooper:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
QBCore.Functions.CreateCallback('statetrooper:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('statetrooper:GetCops', function(source, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "statetrooper" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('statetrooper:server:IsPoliceForcePresent', function(source, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "statetrooper" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('statetrooper:server:TakeOutImpound', function(plate)
    local src = source
...
RegisterNetEvent('statetrooper:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CuffedPlayer = QBCore.Functions.GetPlayer(playerId)
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or Player.PlayerData.job.name == "statetrooper" then
            TriggerClientEvent("statetrooper:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
        end
    end
end)

RegisterNetEvent('statetrooper:server:EscortPlayer', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if (Player.PlayerData.job.name == "statetrooper" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
            TriggerClientEvent("statetrooper:client:GetEscorted", EscortPlayer.PlayerData.source, Player.PlayerData.source)
        else
...
RegisterNetEvent('statetrooper:server:KidnapPlayer', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or
            ...
            TriggerClientEvent("statetrooper:client:GetKidnappedTarget", EscortPlayer.PlayerData.source, Player.PlayerData.source)
            TriggerClientEvent("statetrooper:client:GetKidnappedDragger", Player.PlayerData.source, EscortPlayer.PlayerData.source)
        else
            TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
        end
    end
...
RegisterNetEvent('statetrooper:server:SetPlayerOutVehicle', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
            TriggerClientEvent("statetrooper:client:SetOutVehicle", EscortPlayer.PlayerData.source)
        else
            TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
        end
    end
...
RegisterNetEvent('statetrooper:server:PutPlayerInVehicle', function(playerId)
    local src = source
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
            TriggerClientEvent("statetrooper:client:PutInVehicle", EscortPlayer.PlayerData.source)
        else
            TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
        end
    end
...
RegisterNetEvent('statetrooper:server:BillPlayer', function(playerId, price)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if Player.PlayerData.job.name == "statetrooper" then
        if OtherPlayer then
            ...
            TriggerEvent('qb-bossmenu:server:addAccountMoney', "statetrooper", price)
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
        end
    end
end)

RegisterNetEvent('statetrooper:server:JailPlayer', function(playerId, time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    ...
    if Player.PlayerData.job.name == "statetrooper" then
        if OtherPlayer then
            ...
            TriggerClientEvent("statetrooper:client:SendToJail", OtherPlayer.PlayerData.source, time)
            TriggerClientEvent('QBCore:Notify', src, Lang:t("info.sent_jail_for", {time = time}))
        end
    end
end)

RegisterNetEvent('statetrooper:server:SetHandcuffStatus', function(isHandcuffed)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
RegisterNetEvent('statetrooper:server:SearchPlayer', function(playerId)
    local src = source
...
RegisterNetEvent('statetrooper:server:SeizeCash', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
        ...
        SearchedPlayer.Functions.RemoveMoney("cash", moneyAmount, "statetrooper-cash-seized")
...
RegisterNetEvent('statetrooper:server:SeizeDriverLicense', function(playerId)
    local src = source
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
...
RegisterNetEvent('statetrooper:server:RobPlayer', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
        local money = SearchedPlayer.PlayerData.money["cash"]
        Player.Functions.AddMoney("cash", money, "statetrooper-player-robbed")
        SearchedPlayer.Functions.RemoveMoney("cash", money, "statetrooper-player-robbed")
...
RegisterNetEvent('statetrooper:server:UpdateBlips', function()
    -- KEEP FOR REF BUT NOT NEEDED ANYMORE.
end)

RegisterNetEvent('statetrooper:server:spawnObject', function(type)
    local src = source
    local objectId = CreateObjectId()
    Objects[objectId] = type
    TriggerClientEvent("statetrooper:client:spawnObject", src, objectId, type, src)
end)

RegisterNetEvent('statetrooper:server:deleteObject', function(objectId)
    TriggerClientEvent('statetrooper:client:removeObject', -1, objectId)
end)

RegisterNetEvent('statetrooper:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    local src = source
...
RegisterNetEvent('statetrooper:server:UpdateCurrentCops', function()
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "statetrooper" and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("statetrooper:SetCopCount", -1, amount)
end)
...
RegisterNetEvent('statetrooper:server:showFingerprintId', function(sessionId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local fid = Player.PlayerData.metadata["fingerprint"]
    TriggerClientEvent('statetrooper:client:showFingerprintId', sessionId, fid)
    TriggerClientEvent('statetrooper:client:showFingerprintId', src, fid)
end)

RegisterNetEvent('statetrooper:server:SetTracker', function(targetId)
    local src = source
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('statetrooper:client:SetTracker', targetId, false)
    else
        ...
        TriggerClientEvent('statetrooper:client:SetTracker', targetId, true)
    end
end)

RegisterNetEvent('statetrooper:server:SyncSpikes', function(table)
    TriggerClientEvent('statetrooper:client:SyncSpikes', -1, table)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent("statetrooper:SetCopCount", -1, curCops)
    end
end)
